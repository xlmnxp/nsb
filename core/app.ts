...
function writePosts(): Promise<boolean> {
    return new Promise<boolean>((resolve, reject) => {
        ...
        // انشاء المجلد "posts"
        mkdir(normalize(`${folder_path()}/../${blogInfo().output_folder}/posts`), "0777", (err: any) => {
            if (err) {
                reject(err);
                return;
            }
            // قراءة محتوى المجلد "mdposts"
            readdir(normalize(`${folder_path()}/../mdposts`), (err: any, files: string[]) => {
                if (err) {
                    reject(err);
                    return;
                }
                files.forEach((file: string, index: number) => {
                    // جلب حالة المجلد
                    stat(normalize(`${folder_path()}/../mdposts/${file}`), (err: any, stat: Stats) => {
                        if (err) {
                            reject(err);
                            return;
                        }
...
(async () => {
    try {
        ...
        mkdir(normalize(`${folder_path()}/../${blogInfo().output_folder}/page`), (err: any) => {
            if (err) {
